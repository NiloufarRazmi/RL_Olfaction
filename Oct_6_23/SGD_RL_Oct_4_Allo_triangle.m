function[output] = SGD_RL_Oct_4_Allo_triangle(subID)
...
params.arena = 1; % 1 - triangle , 2 - diamond
...
if params.arena ==1
    params.start = randi(params.numStates/4,1,1);
...
init_states = [1,6,11,16,21,7,13,12,17,22,18,23,19,25];
idx = randi(length(init_states),1,1);
current  = init_states(idx);

%(params.numStates/4) * (odor_port-1) + randi((params.numStates/4),1,1);
