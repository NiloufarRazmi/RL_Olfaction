...
*** TODO Plot state occupancy (of all agents + all states)
*** TODO Plot trajectories of all agents on 2D map
...
*** Silence Cartesian/polar part of the input on both taks + look at the behavior of the agent (x4 experiments)
