...
** Test generalization if agent trained only in upper triangle and later trained in lower triangle
Should learn faster than agent trained only in the lower triangle
** Light cue in the state?
** No ReLU at the last layer?
** Normalizing states/actions?
** Reward at 1 instead of 10?
** TODO Start training when replay buffer is full?
* DONE Debug
** DONE Unit test env
