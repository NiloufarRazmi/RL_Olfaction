...
class Autoencoder(nn.Module):
    ...
    def __init__(self, n_observations, n_actions, bottleneck, n_units=16):
        super().__init__()
        self.mlp = nn.Sequential(
            nn.Linear(n_observations, n_units),
            nn.LeakyReLU(),
...
def auto_encoders(n_observations, n_actions, bottleneck, nHiddenUnits):
    """Define policy and target networks."""
    policy_net = Autoencoder(
        n_observations=n_observations,
        n_actions=n_actions,
