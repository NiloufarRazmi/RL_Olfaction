...
def plot_exploration_rate(epsilons, steps, figpath=None, logger=None):
    ...
    cumsteps = steps.flatten().cumsum(0).cpu()
...
def plot_steps_trained_boxplot(df, figpath=None, logger=None):
    """Plot last steps of trained agent as box plot."""
    fig, ax = plt.subplots()
    # sns.set_theme(style="ticks")  # , palette="pastel")
    # sns.set_theme(style="whitegrid")

    df_select = df[df.Episodes > df.Episodes.max() - 50]  # Last 50 datapoints
    ...
    fig.patch.set_facecolor("white")
    if figpath:
        figfullpath = figpath / "steps-boxplot.png"
        fig.savefig(figfullpath, bbox_inches="tight")
        if logger:
            msg = f"Saved figure to: {figfullpath.absolute()}"
            print(msg)
