...
- All states need to be visited by the agent to propagate the prediction of getting a future reward \to\nbsp{}not what happens in the brain
...
\footnotesize
#+end_export
- Behavioral + neural model
- Neural network that maps states to actions
- Extract features from the simulation data
- Not all states need to be visited by the agent to propagate the prediction of getting a future reward \to\nbsp{}closer to the brain
...
:BEAMER_act: [<+->]
:END:
- We record in the LEC because it encodes spatial & olfactory information
- Reinforcement Learning is a useful framework to model behavior involving rewards and learning
- The joint representation is needed to solve an odor-place association task
- We hope to use Deep Reinforcement Learning to better understand the representations that are at play in this type of task
* Acknowledgments
:PROPERTIES:
:UNNUMBERED: notoc
:END:
