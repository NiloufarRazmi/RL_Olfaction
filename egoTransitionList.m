function [transitionList] = egoTransitionList(params, states, walls)
...
states = [states;states+25;states+50;states+75];
params.numStates = size(states,1) * size(states,2);
transitionList = zeros(params.numStates, 3);
    ...
    for context = 1:params.numContexts
        ...
        for state = 1:params.numStates
            ...
            if state <26
                ...
                transitionList(state, 2) = states(row +15 , col );

                % Right column
                transitionList(state, 3) = states(row +10 , col);


                % Facing Down:
            elseif state < 51 && state > 25

                % Up column
                if row == params.rows +25
                    transitionList(state, 1) = state;
                else
                    transitionList(state, 1) = states(row + 1, col);
                end
                ...
                % Left column
                transitionList(state, 2) = states(row +5 , col );
            ...
            elseif state < 76 && state > 50
                ...
                % Left column
                transitionList(state, 2) = states(row -10,col );

                % Right column
                transitionList(state, 3) = states(row -5,col );
            ...
            elseif state > 75
                ...
                % Right column
                transitionList(state, 3) = states(row -15,col);
            end
